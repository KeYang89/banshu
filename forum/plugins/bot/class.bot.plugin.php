...
class BotPlugin extends Gdn_Plugin {
    ...
    public function bot_shave_handler($bot) {
        if ($bot->match('fuck')) {
            $bot->setReply($bot->mention().' :hushed:');
        }
        if ($bot->match('asshole')) {
            $bot->setReply($bot->mention().' :no_mouth:');
        }
        if ($bot->match('ass')) {
            $bot->setReply($bot->mention().' :worried:');
        }
        if ($bot->match('butt')) {
            $bot->setReply($bot->mention().' poopybutthole from Rick and Morty hahaha');
        }
    }
