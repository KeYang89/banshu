<?php
/**
 * @var \SBWebApplication\View\View $view
 * @var array $config
 * @var int $total
 * @var int $page
 * @var string $search
 * @var string $title
 * @var \SBWebApplication\Site\Model\Node $node
 * @var \SB\Userprofile\User\ProfileUser[] $profileUsers
 */

$view->script('userprofiles', 'sb/userprofile:app/bundle/userprofiles.js', ['vue']);
// Grid
...
<div id="userprofile-profiles">
	...
	<div class="uk-grid uk-grid-match <?= $grid ?>" data-uk-grid-match="{target: '>.uk-panel'}" data-uk-grid-margin>
		<?php foreach ($profileUsers as $profileUser) : ?>
			<div>
				<?= $view->render(sprintf('sb/userprofile/templates/%s.php', $config['list']['template']),
