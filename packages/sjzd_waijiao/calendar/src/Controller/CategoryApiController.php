<?php
    namespace SJZDwaijiao\Calendar\Controller;

    use SBWebApplication\Application as App;
    use SJZDwaijiao\Calendar\Model\Category;
    use SJZDwaijiao\Calendar\Model\Event;
    ...
    class CategoryApiController
    {
        ...
        public function deleteAction($id)
        {
            ...
            $query->where(function ($query) use ($id) {
                $query->orWhere(['category_id' => (int) $id]);
            });

            if ($query->count() > 0) {
                App::abort(403, __("One of the selected categories has already events."));
            }
